...
describe('Test like operator', () => {
  ...
  it('Test statement with mutiple %', async () => {
    ...
    expect(parser.parseSql(`WHERE title not like '%'`)).toStrictEqual({ title: { $not: new RegExp('.*', 'i') } });
    expect(parser.parseSql(`WHERE title like '%%'`)).toStrictEqual({ title: { $regex: ".*", $options: "i" } });
    expect(parser.parseSql(`WHERE title like '%%%'`)).toStrictEqual({ title: { $regex: ".*", $options: "i" } });
    expect(parser.parseSql(`WHERE title not like '%%%'`)).toStrictEqual({ title: { $not: new RegExp('.*', 'i') } });
    expect(parser.parseSql(`WHERE title like '%%%%'`)).toStrictEqual({ title: { $regex: ".*", $options: "i" } });
  ...
  it('Test statement with /%', async () => {
    ...
    expect(parser.parseSql(`WHERE title not like '/%Man%trees/%'`)).toStrictEqual({ title: { $not: new RegExp('%Man.*trees%', 'i') } });
    expect(parser.parseSql(`WHERE title like '%%Man/%%%trees%%%%'`)).toStrictEqual({ title: { $regex: ".*Man%.*trees.*", $options: "i" } });
    expect(parser.parseSql(`WHERE title like '/%%Man/%/%%trees%%/%%'`)).toStrictEqual({ title: { $regex: "^%.*Man%%.*trees.*%", $options: "i" } });
    expect(parser.parseSql(`WHERE title not like '/%%Man/%/%%trees%%/%%'`)).toStrictEqual({ title: { $not: new RegExp('^%.*Man%%.*trees.*%', 'i') } });
    expect(parser.parseSql(`WHERE title like '%/%%/%%'`)).toStrictEqual({ title: { $regex: ".*%.*%.*", $options: "i" } });
  ...
  it('Test statement with special characters', async () => {
    ...
    expect(parser.parseSql(`WHERE name not like 'abc?%'`)).toStrictEqual({ name: { $not: new RegExp('^abc\\?', 'i') } });
    expect(parser.parseSql(`WHERE name like 'a\\%'`)).toStrictEqual({ name: { $regex: "^a\\\\", $options: "i" } });
    expect(parser.parseSql(`WHERE name not like 'a\\%'`)).toStrictEqual({ name: { $not: new RegExp('^a\\\\', 'i') } });
    expect(parser.parseSql(`WHERE name like '%$, (, ), *, +, ., [, ], ?, \\, ^, {, }, |%'`)).toStrictEqual(
      { name: { $regex: ".*\\$, \\(, \\), \\*, \\+, \\., \\[, \\], \\?, \\\\, \\^, \\{, \\}, \\|.*", $options: "i" } }
    );
    expect(parser.parseSql(`WHERE name not like '%$, (, ), *, +, ., [, ], ?, \\, ^, {, }, |%'`)).toStrictEqual(
      { name: { $not: new RegExp('.*\\$, \\(, \\), \\*, \\+, \\., \\[, \\], \\?, \\\\, \\^, \\{, \\}, \\|.*', 'i') } }
    );
  });
});
